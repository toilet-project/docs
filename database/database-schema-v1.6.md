...
erDiagram
    ...
    APP_USER ||--o{ COORDINATE_QUALITY_REVIEW : reviews
```
...
| `coordinate_quality_review` | 동일 좌표 그룹의 관리자 확인 상태 | API Flyway `V5__create_coordinate_quality_review.sql` |
...
| `coordinate_quality_review` | `group_key`, `status`, `reviewed_by_user_id` | 좌표 자체를 자동 변경하지 않고 그룹 검토 상태와 메모만 보존 |
...
| 7 | API Flyway V5 | 중복 좌표 품질 검토 | 직접 보정 이력을 위해 `coordinate_revision.report_id`를 nullable로 확장 |
...
| 버전 | 일자 | 변경 |
| --- | --- | --- |
| v1.6 | 2026-08-31 | 동일 좌표 그룹 검토 상태와 관리자 직접 보정 이력 모델 추가 |
