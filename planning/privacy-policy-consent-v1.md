...
- OAuth 결과로 기존 회원 여부를 먼저 식별한다. 최신 필수 동의가 있는 기존 회원은 별도 연령 확인 없이 바로 로그인하고, 신규 회원에게만 가입 동의와 만 14세 이상 확인을 한 번 받는다.
...
sequenceDiagram
    ...
    alt 기존 회원이며 최신 필수 동의 완료
        A-->>W: HttpOnly 인증 쿠키 + 로그인 완료
    else 신규 회원 또는 정책 재동의 필요
        A-->>W: HttpOnly 인증 쿠키 + 필수 동의 필요 상태
        W->>U: 필요한 필수 항목을 개별 표시
        U->>W: 신규 가입 시 이용약관·개인정보 수집·만 14세 이상 동의
        W->>A: 정책 key·현재 version 동의 제출
        A->>D: 동의 버전·동의 시각 저장, 계정 활성화
    end
```

필수 항목을 하나라도 선택하지 않으면 신규 회원 기능은 활성화하지 않는다. 선택 동의는 필수 동의와 시각적으로 분리하며 기본 선택하지 않는다. 만 14세 이상 확인은 가입 시 한 번 기록하고 일반 로그인 때 반복하지 않는다. 이용약관·개인정보 정책의 필수 버전이 바뀐 경우에만 기존 사용자에게 변경된 정책 재동의를 요청한다.
