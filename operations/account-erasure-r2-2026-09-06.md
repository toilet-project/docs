...
검증일: 2026-09-06. API·배치·docs의 로컬 `feature/account-withdrawal-retention` 작업이다. **운영 배포·회원 파기·실제 회원정보 외부 전송은 하지 않았다.** 원격 feature push/PR도 이번 단계에서 하지 않았다.
...
현재 런타임 S3 키 등록은 사용자에게 요청한 상태이며 실제 서버 연결 확인 전이다. 암호화 운영 키 생성·배포 secret 주입도 미완료다. `ACCOUNT_RETENTION_ENABLED`, `ACCOUNT_ERASURE_ENABLED`, `ERASURE_LEDGER_ENABLED`는 활성화하지 않는다.
...
R2 왕복은 Wrangler 인증을 사용한 **가상 데이터**다. Java 런타임의 실제 S3 자격 증명·권한·키링 연결 검증과 구분한다. 가상 암호화 fixture의 테스트 키를 운영에 사용하지 않는다. 원격 가상 객체는 삭제했고 로컬 테스트 증빙만 남겼다.
...
운영 전 순서: 제한된 S3 키·별도 암호화 키 준비 → 실제 최장 백업 기간/대장 보존·잠금·키 복구 정책 확정 → 복원 스크립트 통합 및 격리 리허설 → 정책·운영 동일 스키마·권한 검증 → 별도 승인 후 원격 반영/배포/활성화. 정상 사용자를 테스트 목적으로 삭제하지 않는다.
