...
7. 22:53 KST 실제 재부팅 후 서비스가 복귀했으나 배포 SSH의 런타임 디렉터리 생성 순서 오류를 발견했다. 시작 순서/사전 생성을 수정해 SSH 인증 및 실패 유닛0을 확인했다. 두 번째 재부팅은 추가 승인 대기이며 실제 구버전 복구·호스트 전체 장애 독립 감시와 구분한다.

## 재부팅·정리의 검증 경계

새 부팅 ID와 DHCP 예약 주소 유지, Tunnel 연결4개, 컨테이너 자동 시작, 공개 회귀9건을 확인했다. API 준비 초기에는 연결 reset이 있었으므로 무중단이 아니다. 배포 SSH만 복구했고 추가 앱/DB 재시작은 없었다. 이후 개인 Access SSH와 내부 포워드를 통한 배포 SSH 명령 실행 성공, 23:00 KST 정기 감시 종료0을 확인했다. 이는 수정 후 부팅 자동 시작이나 GitHub runner의 새 Service Auth 인수를 대신하지 않는다.
