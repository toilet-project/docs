...
22:53 KST 재부팅 후 DHCP 예약 주소·컨테이너·Tunnel·개인 SSH가 돌아왔고 공개 회귀9건을 통과했다. 배포 전용 SSH는 기본 SSH의 런타임 디렉터리 생성보다 먼저 시작해 실패했다. 시작 순서와 디렉터리 사전 생성을 보완한 뒤 해당 SSH만 시작해 인증 성공·실패 유닛0을 확인했다. 수정 로직4건·시험키 정리 안전성5건도 통과했다. **수정 후 두 번째 재부팅 검증은 추가 승인 대기**이며 첫 재부팅이 모두 정상이라고 기록하지 않는다. 재부팅 후 Redis 세션 소멸에 따른 새 로그인은 별도 확인한다.
