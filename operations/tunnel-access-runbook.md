...
승인된 테스트 경로는 삭제했고 복구 자료는 보존했습니다. 현재 커넥터 이름에 `test`가 포함돼 있어도 운영 경로를 처리하므로 이름만 보고 삭제해서는 안 됩니다. 삭제 후 공개 회귀9건·외부 개인 SSH·서비스 활성/실패 유닛0을 확인했습니다. 새 관리자 로그인과 runner Service Auth 재검증은 별도입니다. 토큰 만료 알림 타이머가 존재하는 것과 Tunnel 전체 장애의 독립 외부 감시는 다른 항목입니다.
