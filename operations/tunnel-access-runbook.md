...
| Workbench | 로컬 SSH 포워드 → MySQL loopback | 내부·외부 handshake·서버 DB 읽기 인증·사용자 Test Connection 성공 |

초기 API·관리자 사용자 인수는 공급자 미지정이었으나, 시험 자원 정리 후 Google 새 관리자 로그인과 대시보드·배치 이력·제보 목록 조회를 추가 확인했습니다. Google 공급자 SSO 강제 종료·Kakao·쿠키/만료/권한 실패 시험이 모두 끝났다는 뜻은 아닙니다.
...
- DHCP 고정 할당은 사용자 등록 후 두 번째 재부팅까지 예약 주소 유지를 확인했습니다. 별도 강제 lease 갱신 시험은 아닙니다.
...
- [x] 운영자 Workbench Test Connection으로 DB 인증 확인 (사용자 성공 확인)
- [x] 서버 측 현재 운영 자격증명으로 DB 인증·현재 계정 읽기 확인 (Workbench GUI와 별개)
- [ ] 공급자별 OAuth·권한/만료 세부 인수
- [x] 정리 후 Google 새 관리자 로그인·대시보드·배치 이력·제보 검토 읽기 인수
- [x] 정리 후 API/Batch/Admin runner Service Auth 재인수·600초 종료/재접속·임시 키 정리 (각 attempt2 성공)
- [x] 실제 대상 인증서 staging 갱신 종료0·운영 인증서/설정 불변 확인
- [ ] 다음 정기 배치 결과와 정기 타이머 결과 확인
- [x] Tunnel 감시 운영 설치·시험 알림1건 HTTP204·정기 건강 점검 성공
- [x] 수정 후 두 번째 재부팅·서비스 자동 복귀·배포 SSH 재시도0회·DB/공개 회귀 확인
- [ ] 구버전 rollback·장애 중 실제 복구 시험 (정상 재부팅과 구분)
- [x] 명시한 시험 자원 정리 승인·docs 시험 Secret4개 제거·시험 workflow 비활성화
- [x] 서버 hostname 시험키1줄 제거·나머지 키 보존·시험 listener 비활성화·SSH 재검증
- [x] 승인된 Cloudflare 시험 DNS4개·route4개·앱3개·전용 정책1개·토큰1개 정리 및 운영 목록 보존 확인
- [x] [최신 서비스 아키텍처 v4](../architecture/architecture-v4.md) 문서와 그림의 통합 갱신

승인된 테스트 경로는 삭제했고 복구 자료는 보존했습니다. 현재 커넥터 이름에 `test`가 포함돼 있어도 운영 경로를 처리하므로 이름만 보고 삭제해서는 안 됩니다. 삭제 후 공개 회귀9건·외부 개인 SSH·서비스 활성/실패 유닛0을 확인했고 Google 새 관리자 로그인도 추가 확인했습니다. runner Service Auth 재검증은 별도 근거로 관리합니다. 토큰 만료 알림 타이머가 존재하는 것과 Tunnel 전체 장애의 독립 외부 감시는 다른 항목입니다.

### 실제 재부팅에서 발견한 문제

22:53 KST 첫 재부팅에서 배포 SSH가 런타임 디렉터리 생성보다 먼저 시작해 실패했다. 시작 순서와 디렉터리 사전 생성을 보완하고 추가 승인 후 **23:07 KST 두 번째 재부팅을 완료**했다. 배포 SSH는 수동 시작 없이 active, 재시도0회·종료상태0으로 자동 기동했다. DHCP 예약 주소 유지, 컨테이너6개·Tunnel4연결·개인/배포 SSH 인증, 관리자 건강 상태UP·DB 읽기 인증·공개 회귀9건도 통과했다. 첫 재부팅 오류는 해결됐으나 무중단 또는 장애 중 구버전 복구 시험으로 확대하지 않는다. 재부팅 후 Redis 세션 소멸에 따른 새 로그인은 별도 확인한다.

[전체 WBS #73](https://github.com/toilet-project/docs/issues/73) · [복구·감시·정리 #78](https://github.com/toilet-project/docs/issues/78).

## 추가 점검 결과

- 2026-09-08 운영 인증12건·격리29건 및 독립 외부 감시45건 통과. GitHub 외부 감시 활성화·Discord 저장 확인·정상 무알림 점검과 cache 저장 완료. [추가 인수 및 남은 항목](tunnel-safe-acceptance-2026-09-08.md), [감시 중지·운영 방법](external-monitor/README.md).

- 마감 시점 공개 API·미인증 차단·허용/비허용 CORS·양쪽 OAuth 시작 경로 9건 재검증 통과. 로그인 완료·세션 만료 인수와는 구분한다.
- Tunnel 준비 상태 연동 및 Discord 발송 실패 후 재시도 보완은 격리 통합9건·검사기4건 통과 후 운영 설치했다. 시험 알림1건 HTTP204, 22:34 KST 정기 건강 점검 종료0을 확인했다. 실제 운영 장애를 주입한 검증은 아니다.
- API 인증서 staging 갱신 dry-run은 종료0으로 통과했고 운영 인증서·Nginx·갱신 설정은 불변이다. 실행 후 공개 회귀9건도 통과했다.
- 당일 정기 백업·격리 복원 작업은 종료 코드 0을 확인했다. 보관 점검의 종료 코드 2는 `HOLD_LEGACY_METADATA`로, 기존 메타데이터 없는 백업 삭제를 보류한 결과다. 자동 삭제 완료나 무조건 정상으로 처리하지 않는다.
- 초기 DB 컨테이너 설정과 현재 운영 API의 DB 비밀번호 설정은 다를 수 있다. 실제 운영 자격증명의 인증 성공을 확인했으며 DB 비밀번호 변경/초기화는 하지 않았다. 초기 환경값을 현재 유효한 비밀번호로 단정하지 않는다.
