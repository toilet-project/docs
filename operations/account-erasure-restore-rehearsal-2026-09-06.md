...
- 확인한 정기 백업 디렉터리에 암호화 덤프 10개가 있었다. 2026-09-06 사용자는 별도로 내려받거나 다른 곳에 복사한 DB 백업이 없다고 확인했다. 이 확인은 서버의 다른 디렉터리·스냅샷·binlog·Redis 사본까지 없다는 의미는 아니다.
- 실제 운영 MySQL 바이너리는 8.0.46이다. 이번 CI 격리 검증도 같은 MySQL 버전을 사용한다.
- 추가 읽기 전용 조회에서 `@@log_bin=1`, `@@binlog_expire_logs_seconds=2592000`(30일), `@@binlog_expire_logs_auto_purge=1`을 확인했다. binlog의 실제 파일 목록·최고령 파일·내용은 조회하지 않았다. 이는 설정값이지 모든 로그가 정확히 30일 내 제거됨을 실측한 결과는 아니다. 덤프뿐 아니라 변경 로그와 이를 재생하는 복구 절차도 보관 범위에 포함해야 한다.
...
후속 [보관·정리 정책안 v1](account-erasure-retention-policy-v1.md)에 32일 최초 검토 기준과 dry-run 보류 조건을 구체화했다. 운영 자동 삭제는 아직 연결하지 않았다.
...
별도 수동 백업 사본이 없다는 사용자 확인은 받았다. 다만 활성화된 binlog의 30일 보관 설정과 성공한 백업 뒤에만 실행되는 덤프 정리 조건 때문에, 덤프의 기본값 14일만으로 R2 보관 기간을 확정할 수 없다. 다음 단계는 실제 사본·로그의 만료 보장과 삭제 완료 확인을 기준으로 조건부 대장 정리 정책을 확정하는 것이다. 이번 확인에서는 파일 삭제, MySQL 설정 변경, R2 잠금·자동 만료 설정을 하지 않았다. 이 작업은 법적 적합성을 확정하는 검토를 대체하지 않는다.
